...
namespace WyyMusicConvertGui
{
    public class NeteaseCryptoMusic
    {
        ...
        public static NeteaseCryptoMusic FromFile(string path)
        {
            try
            {
                using (var fs = File.Open(path, FileMode.Open))
                {
                    ...
                    var metadata_object = JsonSerializer.Deserialize<NeteaseCryptoMusicMetaData>(metadata, NeteaseCryptoMusicMetaDataSourceGenerationContext.Default.NeteaseCryptoMusicMetaData);

                    //skip crc32&gap2
                    fs.Seek(9, SeekOrigin.Current);

                    //ImageSource coverimage_source;
                    BitmapImage coverImageSource;
                    ...
                    if (cover_data_size != 0)
                    {
                        ...
                        using (var stream = new MemoryStream(cover_data))
                        {
                            coverImageSource.BeginInit();
                            coverImageSource.StreamSource = stream;
                            coverImageSource.CacheOption = BitmapCacheOption.OnLoad;
                            coverImageSource.EndInit();
                        }
                        isEnabedImages = true;
                    }
                    else
                    {
                        using (var stream = File.Open("music.png",FileMode.Open))
                        {
                            ...
                            using (MemoryStream memoryStream = new MemoryStream())
                            {
                                ...
                                //coverImageSource = ImageSource.FromStream(() => { return new MemoryStream(coverimage_bitmap); });
                                coverImageSource = new BitmapImage();
                                using (var stream1 = new MemoryStream(coverimage_bitmap))
                                {
                                    coverImageSource.BeginInit();
                                    coverImageSource.StreamSource = stream;
                                    coverImageSource.CacheOption = BitmapCacheOption.OnLoad;
                    ...
                    return new NeteaseCryptoMusic(path, metadata_object, coverImageSource, coverimage_bitmap, isEnabedImages);
                }
        ...
        public static NeteaseCryptoMusicMetaData MetaDataFromFile(string path)
        {
            try
            {
                using (var fs = System.IO.File.Open(path, FileMode.Open))
        ...
        public bool WriteDecryptMusic(string outdir)
        {
            ...
            if (System.IO.Path.Exists(NewFilePath) && GlobalVars.Configs.OverwriteFile == false)
            {
                ...
                if (GlobalVars.Configs.DeleteOldFile)
                {
            ...
            try
            {
                using (var fs = System.IO.File.Open(Path, FileMode.Open))
                {
                    ...
                    tagfile.Tag.AlbumArtists = [MetaData.Artist[0].ArtistName];
